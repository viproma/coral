/**
\file
\brief  Defines the dsb::bus::ISlaveControlMessenger interface and the two
...
public:
    explicit PendingSlaveControlConnection(
        std::shared_ptr<PendingSlaveControlConnectionPrivate> p = nullptr) DSB_NOEXCEPT;
    PendingSlaveControlConnection(PendingSlaveControlConnection&&) DSB_NOEXCEPT;
    PendingSlaveControlConnection& operator=(PendingSlaveControlConnection&&) DSB_NOEXCEPT;
    ~PendingSlaveControlConnection() DSB_NOEXCEPT;
    ...
    */
    ...
    called.
    */
    operator bool() const DSB_NOEXCEPT;
...
public:
    /// Default constructor.
    SlaveControlConnection() DSB_NOEXCEPT;
    ...
    /// Move assignment.
    SlaveControlConnection& operator=(SlaveControlConnection&&) DSB_NOEXCEPT;
...
public:
    ...
    */
    virtual ~ISlaveControlMessenger() DSB_NOEXCEPT { }
    ...
    */
    ...
    Possible error conditions are:
      ...
      - `dsb::error::sim_error::cannot_perform_timestep` (non-fatal): The slave
...
Possible error conditions are:
  ...
  - `std::errc::timed_out`: The slave has become unreachable and the connection
        ...
        ConnectToSlave().
  - `dsb::error::generic_error::operation_failed`: The operation failed
...
\throws dsb::error::ProtocolNotSupported if the slave requested an unsupported
    protocol version.
